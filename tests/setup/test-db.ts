...
export const prisma = globalForPrisma.prisma ?? new PrismaClient({
  datasources: {
    db: {
      url: process.env.DATABASE_URL || 'postgresql://postgres:postgres@localhost:5432/newfl',
    },
  },
})
...
export async function createTestVehicle(data: Partial<any> = {}) {
  ...
  return await prisma.vehicle.create({
    data: {
      ...
      status: data.status || 'ACTIVE',
      paymentModel: data.paymentModel || 'DRIVER_REMITS',
...
export async function createTestDriver(data: Partial<any> = {}) {
  ...
  return await prisma.driver.create({
    data: {
      ...
      homeAddress: data.homeAddress || '123 Test St',
      nextOfKin: data.nextOfKin || 'Jane Doe',
      nextOfKinPhone: data.nextOfKinPhone || '+1234567891',
