...
const MultiSelect: React.FC<DropdownProps> = ({ id }) => {
  ...
  const select = (index: number, event: React.MouseEvent) => {
    ...
    if (option && !option.selected) {
      ...
      option.element = event.currentTarget as HTMLElement;
  ...
  const remove = (index: number) => {
    ...
    if (selectedIndex !== -1 && newOptions[index]) {
  ...
  const selectedValues = (): string[] => {
    return selected.map((option) => options[option]?.value).filter((v): v is string => v !== undefined);
  };
